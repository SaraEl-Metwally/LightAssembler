...
public:
   ...
   #ifdef largeintlib
   inline void insert(const kmercode_length &key)
   {
      insert(reinterpret_cast<const unsigned char*>(&key),static_cast<std::size_t>(sizeof(key)));
   }
   #endif
   #ifdef _LP64
   inline void insert(const __uint128_t &key)
   {
      insert(reinterpret_cast<const unsigned char*>(&key),static_cast<std::size_t>(sizeof(key)));
   }
   #endif
   inline void insert(const uint64_t &key)
   {
      insert(reinterpret_cast<const unsigned char*>(&key),static_cast<std::size_t>(sizeof(key)));
   }
   ...
   #ifdef largeintlib
   inline bool contains( const kmercode_length &key) const
   {
      return contains(reinterpret_cast<const unsigned char*>(&key),static_cast<std::size_t>(sizeof(key)));
   }
   #endif
   #ifdef _LP64
   inline bool contains( const __uint128_t &key) const
   {
      return contains(reinterpret_cast<const unsigned char*>(&key),static_cast<std::size_t>(sizeof(key)));
   }
   #endif
